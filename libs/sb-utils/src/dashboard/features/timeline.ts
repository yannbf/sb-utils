...
import {
  escapeHtml,
  formatGapDuration as _formatGapDurationLib,
} from '../lib/format'
import {
  formatDelta,
  hasErrorPayload,
...
export function setupTimeline(
  ...
  const Timeline = (function () {
    ...
    const DOT_R = 4
    const LIFESPAN_H = 10
    // Vertical breathing room inside each swim lane: the dot row sits
    // DOT_Y_OFFSET below the lane top, and LANE_PAD_BOTTOM is reserved
    // below the lowest label before the next lane begins.
    const DOT_Y_OFFSET = 15
    ...
    const st: any = {
      ...
      // Whether to group lanes by user — set automatically each render
      // from `hasMultipleUsers` (on only when the data spans 2+ users).
      groupByUser: false,
      ...
      // Per-render lane geometry: { lanes, heights[], tops[], totalH }.
      // Each lane is sized to ITS OWN longest label (not the global max),
      // so short-label sessions sit close together. Set in render().
      laneLayout: { lanes: [], heights: [], tops: [], totalH: 0 },
      segments: null,
    }
    ...
    function init() {
      ...
      contentCanvas.addEventListener('wheel', onContentWheel, {
        passive: false,
      })
      window.addEventListener('mouseup', onWindowUp)
      window.addEventListener('mousemove', onWindowMove)
      window.addEventListener('resize', () => {
        if (state.view === 'timeline') invalidate()
      })
      mainEl.addEventListener('scroll', () => {
        /* native scroll, no redraw needed */
      })
      ...
      minimapCanvas.addEventListener('wheel', onMinimapWheel, {
        passive: false,
      })
    ...
    function rebuildSegments() {
      ...
      const segs = _buildSegments(stamps, {
        collapseGaps: st.collapseGaps,
        gapSegDispMs: GAP_SEG_DISP_MS,
      })
      if (segs.length === 0) {
        const [first, last] = dataRange()
        st.segments = [
    ...
    function srcToDisp(srcMs) {
      ...
      if (srcMs <= segs[0].srcStart)
        return segs[0].dispStart + (srcMs - segs[0].srcStart)
      const lastSeg = segs[segs.length - 1]
      if (srcMs >= lastSeg.srcEnd)
        return lastSeg.dispEnd + (srcMs - lastSeg.srcEnd)
      for (const s of segs) {
        if (srcMs >= s.srcStart && srcMs <= s.srcEnd) {
          if (s.type === 'data') {
            ...
            const frac =
              (srcMs - s.srcStart) / Math.max(1, s.srcEnd - s.srcStart)
    ...
    function dispToSrc(dispMs) {
      const segs = st.segments
      if (dispMs <= segs[0].dispStart)
        return segs[0].srcStart + (dispMs - segs[0].dispStart)
      const lastSeg = segs[segs.length - 1]
      if (dispMs >= lastSeg.dispEnd)
        return lastSeg.srcEnd + (dispMs - lastSeg.dispEnd)
      for (const s of segs) {
        if (dispMs >= s.dispStart && dispMs <= s.dispEnd) {
          if (s.type === 'data') {
            ...
            const frac =
              (dispMs - s.dispStart) / Math.max(1, s.dispEnd - s.dispStart)
    ...
    function timeToX(t, width) {
      ...
      return (
        LABEL_COL_W +
        ((d - st.viewStartDisp) / (st.viewEndDisp - st.viewStartDisp)) *
          contentW
      )
    }
    function xToDisp(x, width) {
      ...
      return (
        st.viewStartDisp +
        ((x - LABEL_COL_W) / contentW) * (st.viewEndDisp - st.viewStartDisp)
      )
    }
    ...
    function formatAxisTime(ms, includeSeconds) {
      return st.normalize
        ? formatElapsed(ms)
        : formatClockTime(ms, includeSeconds)
    }
    ...
    function resize() {
      ...
      const totalH = Math.max(
        st.laneLayout.totalH + 8,
        mainEl.clientHeight || 400,
      )
    ...
    function render() {
      ...
      // Group by user automatically when the data spans 2+ users. Set
      // BEFORE rebuildSegments — visibleSessions / buildOriginIndex read
      // st.groupByUser. With a single user the timeline stays flat.
      st.groupByUser = hasMultipleUsers.value
      rebuildSegments()
      renderEmptyState()

      // Per-render lane geometry: each lane sized to its OWN longest label
      // (so a short-label session isn't bloated by some other lane's long
      // name) + room for the sticky label column text.
      st.laneLayout = computeLaneLayout()
    ...
    function renderEmptyState() {
      ...
      if (
        sessions.length === 0 ||
        sessions.every((s) => s.events.length === 0)
      ) {
        emptyEl.classList.add('has-action')
        const parts: string[] = []
        if (state.activeFilter !== 'all')
          parts.push('type "' + state.activeFilter + '"')
        if (state.activeSession)
          parts.push('session ' + state.activeSession.slice(0, 8))
        if (state.hiddenTypes.size > 0)
          parts.push(
            state.hiddenTypes.size +
              ' hidden type' +
              (state.hiddenTypes.size !== 1 ? 's' : ''),
          )
        if (state.hiddenSessions.size > 0)
          parts.push(
            state.hiddenSessions.size +
              ' hidden session' +
              (state.hiddenSessions.size !== 1 ? 's' : ''),
          )
        if (state.searchQuery) parts.push('search "' + state.searchQuery + '"')
        const filterDesc =
          parts.length > 0 ? parts.join(', ') : 'the current filters'
        emptyEl.innerHTML =
          '<div class="empty-icon">' +
    ...
    function updateInfo() {
      ...
      zoomInfo.innerHTML =
        'Zoom: <b>' + (zoom < 10 ? zoom.toFixed(1) : Math.round(zoom)) + '×</b>'
      if (state.events.length > 0) {
        ...
        const relSuffix = st.normalize
          ...
          : ' <span style="color:var(--text-dim)">(+' +
            formatRelTime(startSrc, first, false) +
            ' – +' +
            formatRelTime(endSrc, first, false) +
            ')</span>'
        rangeInfo.innerHTML =
          'Range: <b>' +
          formatAxisTime(startSrc, true) +
          ' – ' +
          formatAxisTime(endSrc, true) +
          '</b>' +
          relSuffix
      } else {
      ...
      jumpBtn.classList.toggle(
        'visible',
        !st.followTail && state.events.length > 0,
      )
    ...
    function drawAxis() {
      ...
      for (const seg of st.segments) {
        ...
        const segViewSrcStart = Math.max(
          seg.srcStart,
          dispToSrc(st.viewStartDisp),
        )
      ...
      for (const seg of st.segments) {
        ...
        const x1 =
          LABEL_COL_W +
          ((seg.dispStart - st.viewStartDisp) / viewRange) * (w - LABEL_COL_W)
        const x2 =
          LABEL_COL_W +
          ((seg.dispEnd - st.viewStartDisp) / viewRange) * (w - LABEL_COL_W)
        ...
        axisCtx.lineWidth = 1
        axisCtx.beginPath()
        axisCtx.moveTo(gx1 + 0.5, 0)
        axisCtx.lineTo(gx1 + 0.5, h)
        axisCtx.moveTo(gx2 - 0.5, 0)
        axisCtx.lineTo(gx2 - 0.5, h)
        axisCtx.stroke()
        axisCtx.setLineDash([])
      }
    ...
    function drawContent() {
      ...
      const drawerW =
        drawerEl && drawerEl.classList.contains('open')
          ? drawerEl.clientWidth || 0
          : 0
      ...
      for (let i = 0; i < sessions.length; i++) {
        ...
        // Inter-lane divider — skipped in group-by-user mode so sessions
        // within a user read as one block; the colored group separator
        // (drawn at each user's first lane) delineates users instead.
        if (!st.groupByUser) {
          contentCtx.fillStyle = 'rgba(255,255,255,0.07)'
          contentCtx.fillRect(LABEL_COL_W, yTop + laneH - 1, w - LABEL_COL_W, 1)
        }
        ...
        for (const v of visible) {
          if (
            curGroup.length > 0 &&
            v.x - curGroup[curGroup.length - 1].x > CLUSTER_PX
          ) {
        ...
        const drawEventDot = (e: any, x: number, forcePos: boolean) => {
          ...
          contentCtx.arc(
            x,
            yDot,
            isHover || isSelected ? DOT_R + 2 : DOT_R,
            0,
            Math.PI * 2,
          )
          contentCtx.fill()
        ...
        for (const grp of groups) {
          ...
          const { e: first, x: rawFirstX } =
            repIndex >= 0 ? grp[repIndex] : grp[0]
          ...
          const pillText = '+' + more
          contentCtx.font =
            'bold 10px ui-monospace, SFMono-Regular, Menlo, monospace'
          const pillW = Math.max(
            20,
            (contentCtx.measureText(pillText).width as number) + 10,
          )
          contentCtx.font =
          ...
          // can't be clicked.
          if (pillX + pillW > visibleRight - 4)
            pillX = firstX - DOT_R - 6 - pillW
          ...
          const containsSelected = grp.some(
            (g) => g.e._index === st.selectedIdx,
          )
      ...
      for (let i = 0; i < sessions.length; i++) {
        ...
        if (state.activeSession === lane.sid) {
          ...
          contentCtx.fillRect(0, yTop, LABEL_COL_W, laneH)
          contentCtx.fillStyle = '#6c9ef8'
          contentCtx.fillRect(0, yTop, 2, laneH)
        }
        ...
        if (isUserHeader) {
          ...
          // Dark pill behind the caption (like the event labels) so it
          // stays readable where it spills past the label column onto
          // dots / the group separator line.
          const capW = contentCtx.measureText(capText).width as number
          contentCtx.fillStyle = 'rgba(10,14,20,0.85)'
          contentCtx.fillRect(9, capY - 7, capW + 6, 14)
          contentCtx.fillStyle = uc
          contentCtx.fillText(capText, 12, capY)
        }

        // Top-align the label-column text near the dot row (lanes vary in
        // height now). Under a user caption it starts lower.
        const sidY = isUserHeader ? yTop + DOT_Y_OFFSET + 14 : yTop + DOT_Y_OFFSET + 4
        ...
        contentCtx.font = '10px ui-monospace, SFMono-Regular, Menlo, monospace'
        // Line 2: event count + how long the session ran.
        const span = lane.last - lane.first
        const countLabel =
          lane.kind === 'cache'
            ? lane.events.length + ' op' + (lane.events.length !== 1 ? 's' : '')
            : lane.events.length + ' ev' + (lane.events.length !== 1 ? 's' : '')
        const subLabel =
          lane.events.length >= 2 && span > 0
            ? countLabel + ' · ' + formatGapDuration(span)
            : countLabel
        contentCtx.fillText(subLabel, 12, sidY + 14)
        // Line 3: absolute clock time the session started — lets the user
        // correlate sessions across a user even when "Align starts" stacks
        // them all at elapsed 0. Cache lane has no meaningful start.
        if (lane.kind !== 'cache') {
          contentCtx.fillStyle = dimSession ? 'rgba(95,112,133,0.5)' : '#788aa0'
          contentCtx.fillText(formatClockTime(lane.first, true), 12, sidY + 28)
        }
      }
    }
    ...
    // Widest label among a SINGLE lane's own events (rotated labels render
    // at -45°). Used to size each lane independently so a short-label
    // session isn't stretched to fit some other lane's long event name.
    function laneOwnMaxLabelW(lane: any): number {
      let max = 0
      for (const e of lane.events) {
    ...
    // Height for one lane: the max of (a) the room its own rotated labels
    // need below the dot row, and (b) the room the sticky label column text
    // needs (id + count·duration + start time, plus a user caption on the
    // first lane of a group). Capped so one giant label can't triple it.
    function laneHeightFor(lane: any): number {
      // Label hangs from yLabel (yDot 11 + DOT_R 4 + 8 = 23) down by
      // width·sin(45°); trailing pad gives a little breathing room before
      // the next lane so rows don't feel claustrophobic.
      const labelNeed = DOT_Y_OFFSET + DOT_R + 8 + laneOwnMaxLabelW(lane) * 0.7071 + LANE_PAD_BOTTOM
      const isUserHeader =
        st.groupByUser && lane.kind === 'session' && lane.isFirstOfUser
      // Text column: id @ sidY, count @ +14, start @ +28, then pad.
      // sidY is 26 under a header, else 16.
      const textNeed = (isUserHeader ? DOT_Y_OFFSET + 14 : DOT_Y_OFFSET + 4) + 28 + LANE_PAD_BOTTOM
      return Math.min(LANE_H_MAX, Math.round(Math.max(labelNeed, textNeed)))
    }

    // Per-render lane geometry. Each lane gets its own height; tops are the
    // running offsets; totalH sizes the content canvas.
    function computeLaneLayout(): {
      ...
      const lanes = visibleSessions()
      const heights = lanes.map(laneHeightFor)
      const tops: number[] = []
      let y = 0
      ...
      return { lanes, heights, tops, totalH: y }
    }
    ...
    function eventMatchesActiveFilters(e) {
      if (state.activeFilter !== 'all' && e.eventType !== state.activeFilter)
        return false
    ...
    function findLaneLabelAt(clientX, clientY) {
      ...
      if (x > LABEL_COL_W) return null
      for (const lane of hitMap)
        if (y >= lane.yTop && y <= lane.yBottom) return lane
      return null
    }
    ...
    function onContentMove(e) {
      if (st.contentBoxSelect) {
        const rect = contentCanvas.getBoundingClientRect()
        st.contentBoxSelect.currentX = Math.max(
          LABEL_COL_W,
      ...
      if (st.panDrag) {
        ...
        const w = contentCanvas.clientWidth
        const dispPerPx =
          (st.panDrag.end - st.panDrag.start) / (w - LABEL_COL_W)
      ...
      if (cluster) {
        ...
        if (st.hoveredIdx !== hit.idx) {
          st.hoveredIdx = hit.idx
          invalidate()
        }
    ...
    function onWindowMove(e) {
      ...
      if (d.mode === 'pan') {
        ...
        const newStartSrc = Math.min(
          d.startSrcEnd - 200,
          d.startSrcStart + deltaSrc,
        )
        st.viewStartDisp = srcToDisp(newStartSrc)
      } else if (d.mode === 'resize-right') {
        const newEndSrc = Math.max(
          d.startSrcStart + 200,
          d.startSrcEnd + deltaSrc,
        )
        st.viewEndDisp = srcToDisp(newEndSrc)
      }
      st.followTail = false
    ...
    function updateContentSelectOverlay() {
      ...
      contentSelectEl.style.height =
        (mainEl ? mainEl.clientHeight : contentCanvas.clientHeight) + 'px'
    ...
    function showTooltip(event, clientX, clientY) {
      ...
      const elapsedMs = st.normalize
        ? eventSrc(event)
        : event._receivedAt - first
      ...
      const userHtml = uk
        ? '<div class="tt-row"><span>user</span><span class="val">' +
          escapeHtml(shortUserLabel(uk)) +
          '</span></div>'
        : ''
      tooltipEl.innerHTML =
        '<div class="tt-head">' +
        '<span class="tt-idx">#' +
        (displayIdx || event._index) +
        '</span>' +
        '<div class="tt-badge" style="background:' +
        color.bg +
        '; color:' +
        color.fg +
        '">' +
        ...
        summaryHtml +
        '</div>' +
        '<div class="tt-row"><span>time</span><span class="val">' +
        formatClockTime(event._receivedAt, true) +
        '</span></div>' +
        '<div class="tt-row"><span>elapsed</span><span class="val">+' +
        formatRelTime(elapsedMs, 0, false) +
        '</span></div>' +
        userHtml +
        '<div class="tt-row"><span>since prev</span><span class="val">' +
        sincePrev +
        '</span></div>' +
        selectedDeltaHtml +
      ...
      if (left + tRect.width + 14 > wrapRect.width)
        left = clientX - wrapRect.left - tRect.width - 14
      if (top + tRect.height + 14 > wrapRect.height)
        top = clientY - wrapRect.top - tRect.height - 14
    ...
    function showClusterTooltip(
      ...
      const rows = shown
        .map((ev: any) => {
          ...
          return (
            ...
            '<span class="tt-cluster-dot" style="background:' +
            c.fg +
            '"></span>' +
            '<span class="tt-cluster-name">' +
            escapeHtml(name) +
            '</span>' +
            '<span class="tt-cluster-time">' +
            formatClockTime(ev._receivedAt, true) +
            '</span>' +
            '</div>'
          )
        })
      ...
      const overflowRow =
        overflow > 0
          ? '<div class="tt-cluster-row tt-cluster-more">+' +
            overflow +
            ' more</div>'
          : ''
      tooltipEl.innerHTML =
    ...
    function navigableEventsFor(event) {
      ...
      if (event._source === 'cache-watch') {
        return state.events
          .filter((e) => e._source === 'cache-watch' && keep(e))
          .sort(byTime)
      }
      if (event.sessionId) {
        return state.events
          .filter((e) => e.sessionId === event.sessionId && keep(e))
          .sort(byTime)
      }
      return []
    }
    ...
    function focusSelectedEvent(event: any) {
      ...
      const dotX =
        LABEL_COL_W + ((dotDisp - st.viewStartDisp) / viewRange) * contentW
      if (!shouldPanToFocus({ dotX, canvasW, drawerW, labelColW: LABEL_COL_W }))
        return
    ...
    function toggleCollapseGaps() {
      ...
      const viewD = Math.max(
        500,
        Math.min(total * 1.1, st.viewEndDisp - st.viewStartDisp),
      )
