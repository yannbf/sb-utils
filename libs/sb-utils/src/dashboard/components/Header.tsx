...
function ExpandButton() {
  ...
  return (
    ...
    >
      {/*
        Same 4-arrow expand/collapse pair as the per-JsonView toggle
        in EventCard / TimelineDrawer / CacheView, so the visual
        language is consistent: arrows pointing OUT = expand, arrows
        pointing IN = collapse. This button still operates on event
        CARDS (open/close their bodies); JsonView's own collapse
        state is owned per-card and starts collapsed-to-top so a
        bulk Expand here doesn't try to render every nested key.
      */}
      {on ? (
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
